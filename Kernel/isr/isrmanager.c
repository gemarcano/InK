...
enum event_status : uint8_t { EVENT_INSERT,
    EVENT_COMMIT,
    EVENT_SIGNAL,
    EVENT_DONE };
...
// keep track of the popped event since tasks might be restarted
...
{
    return __perqueue_is_full(&_events[thread->priority]);
}
...
{
    return !__perqueue_is_empty(&_events[thread->priority]);
}
...
{
    _popped[thread->priority] = __perqueue_pop(&_events[thread->priority]);
    return _popped[thread->priority];
}

// Events should be released after lock
void __release_event(thread_t* thread)
{
    __perqueue_pop_commit(&_events[thread->priority]);
    _popped[thread->priority] = NULL;
}
